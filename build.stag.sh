...
npm run build:stag
docker build -f Dockerfile -t registry.cn-hangzhou.aliyuncs.com/uki-web/template-project:test ./dist
docker push registry.cn-hangzhou.aliyuncs.com/uki-web/template-project:test
docker rmi registry.cn-hangzhou.aliyuncs.com/uki-web/template-project:test
